...
func (lfu *LFUCache) Get(key int) int {
	if value, ok := lfu.cache[key]; ok {
		lfu.freq[key]++
		for i, v := range lfu.list {
			if v == key {
				lfu.list = append(lfu.list[:i], lfu.list[i+1:]...)
				lfu.list = append(lfu.list, key)
				break
			}
		}
		return value
...
func (lfu *LFUCache) Put(key int, value int) {
	if _, ok := lfu.cache[key]; ok {
		...
		if len(lfu.list) == lfu.capacity {
			minFreq := lfu.freq[lfu.list[0]]
			for _, v := range lfu.freq {
				if v < minFreq {
					minFreq = v
				}
			}
			for i, v := range lfu.list {
				if minFreq == lfu.freq[v] {
